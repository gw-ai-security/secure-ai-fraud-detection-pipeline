...
- Optional explanation artefacts (later)
...
### Source of Truth
Inference must use the same preprocessing artefact used during training:
...
Threshold changes must be:
- tracked in Git (config file)
...
### Artefact Integrity
- Treat `feature_pipeline.pkl` and model files as **derived artefacts**
...
- Separate read/write permissions for artefacts
